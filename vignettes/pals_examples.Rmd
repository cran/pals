...
How effective is `pal.compress`? Compressing all 50 `kovesi.*` colormaps reduced memory from 352000 to 46000 bytes, a savings of 87%.

In the figure below, the top band is the (mathematically-defined) `cubehelix` colormap function evaluated at 255 colors. The `cubebasis` vector has 17 colors (shown in the middle). These 17 colors are passed into the `colorRampPalette` function and expanded to 255 colors shown in the bottom band.  The maximum squared LUV distance between the individual colors in the two bands is 2.34, which is smaller than the theoretical perceptual difference of roughly 2.5.
```{r}
...
1. Are large regions distinct from each other?
